...
<div>
	<h2 class="page-header">{$aLang.plugin.umanager.admin}</h2>
	
	
	{if $aParams.0=='new'}
		<h3 class="page-sub-header">{$aLang.plugin.umanager.create}</h3>
		{include file=$aTemplatePathPlugin.page|cat:'actions/ActionPage/add.tpl'}
	{elseif $aParams.0=='edit'}
		<h3 class="page-sub-header">{$aLang.plugin.umanager.edit} «{$oPageEdit->getTitle()}»</h3>
		{include file=$aTemplatePathPlugin.page|cat:'actions/ActionPage/add.tpl'}
	{else}
		<a href="{router page='page'}admin/new/" class="page-new">{$aLang.plugin.umanager.new}</a><br /><br />
	{/if}
	...
	<table cellspacing="0" class="table">
		<thead>
			<tr>
				<th width="180px">{$aLang.plugin.umanager.admin_title}</th>
				<th align="center" >{$aLang.plugin.umanager.admin_url}</th>    	
				<th align="center" width="50px">{$aLang.plugin.umanager.admin_active}</th>    	   	
				<th align="center" width="70px">{$aLang.plugin.umanager.admin_main}</th>    	   	
				<th align="center" width="80px">{$aLang.plugin.umanager.admin_action}</th>
			</tr>
		</thead>
		...
		<tbody>
			{foreach from=$aPages item=oPage name=el2} 	
				<tr>
					...
					<td align="center">
						{if $oPage->getActive()}
							{$aLang.plugin.umanager.admin_active_yes}
						{else}
							{$aLang.plugin.umanager.admin_active_no}
						{/if}
					</td>
					<td align="center">
						{if $oPage->getMain()}
